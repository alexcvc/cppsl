...
class ResultOptVal {
 public:
  /**
     * @class ResultOptVal
     * @brief A class that represents a result using std::optional. It provides methods for checking if the result is valid or invalid,
     * retrieving the result value, and getting a default
    * value if no result is present.
     *
     * It is important to note that this class does not inherit from a standard library class to avoid incorrect behavior.
     * It is marked as final so that it cannot be used as a base class
    *. Instead, it contains an std::optional member for storing the result value.
     *
     * @tparam T The type of the result value.
     */
  constexpr ResultOptVal(T const&& t) noexcept : _opt_result_value{t} {}
  ...
  /**
     ...
     */
  [[nodiscard]] constexpr bool valid() const noexcept { return _opt_result_value.has_value(); }
  ...
  /**
    ...
    *. Instead, it contains an std::optional member for storing the result value.
     ...
     * @tparam T The type of the result value.
     */
  [[nodiscard]] constexpr auto get() const -> T { return _opt_result_value.value(); }

  /**
     * @brief Retrieves the result value or a default value if no result is present.
     *
     * This method returns the result value stored in `_opt_result_value`. If there is no result value, it returns a default-constructed
     * value of type `T`.
     *
     * @return The result value, or a default-constructed value of type `T` if no result is present.
     */
  [[nodiscard]] auto get_or() const noexcept -> T { return _opt_result_value.value_or(T()); }
...
struct ResultVarVal : protected std::variant<std::monostate, T> {
  ...
  /**
     ...
     * Represents a result value that can hold a variable or void.
     */
  constexpr ResultVarVal(T const&& t) noexcept : std::variant<std::monostate, T>{t} {}
  ...
  /**
     ...
     */
  constexpr auto get() const noexcept -> T { return (valid() ? std::get<T>(*this) : T()); }
};
...
struct ResultOptional : protected std::optional<T> {
  ...
  /**
     ...
     * a valid value of type T or is empty. This class is designed
    * to be a wrapper around std::optional and adds some convenience methods for checking the validity of the result and accessing
    * the value.
     *
     * Usage:
     * ```
  ...
  /**
     ...
     *
     * This member function returns the value contained in the ResultOptional object. If the object has a value, that value is returned.
     * If the object is empty, a default-constructed value of type T is returned.
     *
     * @tparam T The type of the value.
     * @return The value contained in the ResultOptional object.
     */
  [[nodiscard]] constexpr auto get() const noexcept -> T { return std::optional<T>::value_or(T()); }
};
