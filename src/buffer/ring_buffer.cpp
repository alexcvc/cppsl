...
Ring_Buffer_Ex<Atomic>::Ring_Buffer_Ex(size_t capacity) : cap_(capacity + 1), rbdata_(new uint8_t[capacity + 1]) {}
...
template
class Ring_Buffer_Ex<true>;

template
class Ring_Buffer_Ex<false>;
...
template
class Soft_Ring_Buffer_Ex<std::shared_mutex>;

#endif

template
class Soft_Ring_Buffer_Ex<std::shared_timed_mutex>;
